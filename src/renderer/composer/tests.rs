...
/// 실제 배치 경로(`reflow_line_segs`)에서 줄 머리 금칙을 지키는가. **아직 안 지킨다.**
///
/// `getCursorRect` 가 읽는 줄 정보는 이 함수가 쓴다. 신고된 화면("…걸어갔다" / "." 로
/// 갈려 마침표만 다음 줄에 남는다)은 이 경로로 만들어진다.
///
/// 한 번 고쳤다가 되돌렸다. **한컴 오라클과 정면으로 부딪히기 때문이다.**
///
/// `tests/issue_2214_page_local_repaint.rs` 는 한컴 2020 이 저장한 줄 나눔을 기대값으로
/// 박아 두었는데(`[0, 44, 84, 122, 129]`), 그 문단은 `…대하여 적용한다.` 로 끝나고
/// **129번이 바로 그 마침표다.** 곧 한컴은 그 자리에서 마침표를 혼자 새 줄에 두었다.
/// 금칙을 적용하면 128에서 끊겨 오라클이 깨진다.
///
/// 반면 사용자가 한글 2024 화면으로 보여 준 본문에서는 "…걸어갔" / "다." 로 마침표를
/// 혼자 두지 않는다. 둘 다 문단 끝 마침표인데 동작이 반대다.
///
/// 갈릴 만한 것: 오라클 문단은 **표 칸 안**이고 사용자 문단은 본문이다. 또는 한컴
/// 2020 과 2024 의 차이다. 본문/셀로 갈라 보았지만 hwpx 경로에서는 그 구분이 서지
/// 않아 원칙이 되지 못했다.
///
/// 다음에 이어받을 때: **한글 2024 에서 표 칸 안에 같은 문장을 넣어 보면** 갈린다.
/// 칸에서도 "다." 로 붙는다면 2020 오라클이 낡은 것이므로 기대값을 고치면 된다.
#[test]
#[ignore = "미해결 — 한컴 2020 저장 오라클(#2214)과 충돌한다. 주석에 확인 방법"]
