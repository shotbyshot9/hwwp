...
import { markToggleState } from '../toggle-state.ts';
import { toggleAllToolbars, toggleToolbar } from '../../ui/toolbar-visibility.ts';
...
// 켜짐 표시는 `command/toggle-state.ts` 로 옮겼다 — 도구 상자 접기와 서로 부르는
// 고리를 끊기 위해서다. 예전처럼 이 파일에서 가져다 쓸 수 있게 다시 내보낸다.
export { markToggleState };
...
export const viewCommands: CommandDef[] = [
   ...
   * 도구 상자 접기.
   *
   * 상태는 `ui/toolbar-visibility.ts` 가 들고 설정에 저장한다. 예전에는 이 자리의
   * 클로저 변수에 두었는데, 그러면 제목 줄 단추로 접은 것을 메뉴가 모르고 새로고침하면
   * 되돌아왔다.
   */
  {
  ...
  },
  {
    ...
    label: '서식',
    execute() {
      toggleToolbar('format');
    },
  },
  {
    // 제목 줄 단추와 Ctrl+F1 — 두 줄을 한꺼번에 접는다.
    id: 'view:toggle-toolbars',
    label: '툴바 감추기',
    execute() {
      toggleAllToolbars();
    },
